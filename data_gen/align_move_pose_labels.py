...
"""
Refer from BABEL official repo: https://babel.is.tue.mpg.de/.
...
class BabelAlignPose:
    ...
    def __init__(self, dataset, dense=True, seq_dense_ann_type={}):
        ...
        self.joint_pose_path = os.path.join('/media/yan/TOSHIBA EXT', 'AMASS_Data')  # 'lsblk' cmd list all disk names

        # Get frame-rate for each seq. in AMASS
        feat2fps_path = '../data/featp_2_fps.json'
        self.feat2fps = dutils.read_json(feat2fps_path)

        # Dataset w/ keys = {'X', 'Y1', 'Yk', 'Yov', 'seg_id',  'sid', 'seg_dur'}
        self.d = defaultdict(list)
        for ann in tqdm(self.babel):
            self._align_pose(ann)
    ...
    def _load_seq_feats(self, ft_p):
        '''Given path to joint position features, return them in 30fps'''
        # Sanity check
        fft_p = ospj(self.joint_pose_path, ft_p)
        assert os.path.exists(fft_p)

        # Load seq. fts.
        feat = np.load(fft_p)['poses']    # original: 'joint_pos'
        T, ft_sz = feat.shape

        # Get NTU skeleton joints, todo: some index out of range
        ntu_js = dutils.smpl_to_nturgbd(model_type='smplh', out_format='nturgbd')
        feat = feat.reshape(T, -1, 3)
        feat = feat[:, ntu_js, :]

        # Sub-sample to 30fps
        orig_fps = self.feat2fps[ft_p]
        ft = self._subsample_to_30fps(feat, orig_fps)
        # print(f'Feat. shape = {ft.shape}, fps = {orig_fps}')
        # if orig_fps != 30.0:
        #   self._viz_x(ft)
        return ft

    def _get_per_frm_labels(self, ann, ann_type, seq_dur):
        ''' '''
    ...
    def _compute_dur_samples(self, ann, ann_type, seq_ft, seq_dur, dur=5.0):
        ...
        '''
        #
        y_frm = self._get_per_frm_labels(ann, ann_type, seq_dur)
        ...
        for seg in ann['labels']:
            ...
            seg_feat = seq_ft[seg_st_f: seg_end_f, :, :]

            # Split segment into <n_chunks> <dur>-second chunks, e.g. split seg every 5 seconds, each split -> chunk
            n_frm_per_chunk = 30.0 * dur
            n_chunks = int(np.ceil(seg_feat.shape[0]/n_frm_per_chunk))
            for n_ch in range(n_chunks):

                # Single <dur>-sec. chunk in segment
                ch_st_f = int(n_frm_per_chunk * n_ch)
                ch_end_f = int(min(ch_st_f + n_frm_per_chunk, seg_feat.shape[0]))
                x = seg_feat[ch_st_f: ch_end_f, :, :]

                # Handle case where chunk_T < n_frm_per_chunk
                x_T, n_joint, xyz = x.shape
                x_ch = np.concatenate((x, np.zeros((int(n_frm_per_chunk) - x_T,  n_joint, xyz))), axis=0)

                # Labels for this chunk
                yov = Counter(flatten([y_frm[seg_st_f + n_f] for n_f in range(ch_st_f, ch_end_f)]))
                ...
                for cat in seg['act_cat']:
                    ...
                    seq_samples.append({'seg_id': seg['seg_id'],
                                        'chunk_n': n_ch,
                                        'chunk_dur': round(x_T/n_frm_per_chunk, 3),
                                        'x': x_ch,
                                        'y1': cat,
                                        'yk': seg['act_cat'],
                                        'yov': yov,
    ...
    def _align_pose(self, ann):
        '''Return one sample (one segment) = (X, Y1, Yall)'''

        # Get feats. for seq.
        seq_ft = self._load_seq_feats(ann['feat_p'])
...
for spl in ['train', 'val']:
    ...
    dense_babel = BabelAlignPose(dataset, dense=True)
    # Store Dense BABEL
    ...
    # dataset = [data[sid] for sid in data]
    # extra_babel = BabelAlignPose(dataset, dense=False,
